...
contract assign4Token is Ownable, ERC721 {
    ...
    function _mintAndIncrementSupply(address to, uint tokenId) internal{
        require(lastPublickSaleTokenIndex<LAST_TOKEN_ID,'Token supply limit reached.');
        _mint(to,tokenId);
        totalSupply++;
    }
    
    function setNFTprice(uint price) external onlyOwner {
        salePrice = price;
    }
    ...
    function buyNft(uint quantity) external payable {
        ...
        require(block.timestamp < _lastTransactionTime + 30 days, 'Sale ended.');
